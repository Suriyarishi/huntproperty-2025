...
const AgentDashboardView: React.FC<AgentDashboardViewProps> = ({ onNavigate, userName = 'RISHI KESAVAN S K' }) => {
    ...
    const handleTabChange = (tabId: string) => {
        ...
        if (tabId === 'properties') {
            ...
            setActiveSidebarItem('view-clients');
        }
    };
    ...
    const propertiesSidebarItems = [
        ...
        { id: 'manage-projects', label: 'Manage Project', icon: LayoutDashboard },
    ...
    return (
        <div className="min-h-screen bg-[#F4F7FA] font-sans text-gray-800 pb-20">
            ...
            <div className="max-w-[1240px] mx-auto px-4 space-y-8">
                ...
                <div className="bg-white rounded-[40px] shadow-xl p-10 flex flex-col items-center gap-10 border border-gray-50 overflow-hidden relative">
                    <div className="w-full flex flex-col md:flex-row items-center gap-10 relative z-10">
                        {/* Profile Photo Wrapper */}
                        <div className="flex flex-col items-center gap-4">
                            <div className="relative group">
                                <div className="w-40 h-40 rounded-full overflow-hidden border-4 border-gray-50 shadow-inner bg-gray-100 flex items-center justify-center">
                                    <ImageIcon size={48} className="text-gray-300" />
                                </div>
                                <div className="absolute bottom-2 right-2 bg-[#2FED9A] p-2 rounded-lg shadow-lg cursor-pointer hover:scale-110 transition-transform">
                                    <Plus size={18} className="text-[#1a1c21]" />
                                </div>
                            </div>
                            <button className="text-[10px] font-black uppercase tracking-widest text-[#1a1c21] bg-gray-100 px-4 py-2 rounded-full hover:bg-gray-200 transition-colors">
                                + Add Photo
                            </button>
                        </div>
                        ...
                        <div className="flex-1 space-y-8">
                            <div className="space-y-1 text-center md:text-left">
                                <h1 className="text-4xl font-black text-[#1a1c21] tracking-tight uppercase">Welcome, {userName === 'RISHI KESAVAN S K' ? 'vinay rathore' : userName}</h1>
                                <p className="text-[11px] font-bold text-teal-500 uppercase tracking-[0.4em]">Proprietary Workspace</p>
                            </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                                ...
                                <div className="space-y-4">
                                    <div className="flex items-center justify-between border-b border-gray-100 pb-2">
                                        <h3 className="text-[11px] font-black text-gray-400 uppercase tracking-widest">Statistics</h3>
                                        <span className="text-[10px] font-bold text-gray-300">01-Mar-2026</span>
                                    </div>
                                    <div className="space-y-3">
                                        {stats.map((s, idx) => (
                                            <div key={idx} className="flex items-center justify-between group">
                                                <span className="text-[13px] font-bold text-gray-500 group-hover:text-[#1a1c21] transition-colors">{s.label}:</span>
                                                <span className="text-[13px] font-black text-[#1a1c21]">{s.value}</span>
                                            </div>
                                        ))}
                                    </div>
                                </div>
                                ...
                                {/* Identity Snippet */}
                                <div className="bg-[#1a1c21] rounded-[30px] p-6 text-white space-y-4 shadow-2xl relative overflow-hidden">
                                    <div className="absolute -right-4 -top-4 w-24 h-24 bg-teal-500/10 rounded-full blur-2xl"></div>
                                    <div className="flex items-center justify-between">
                                        <div className="space-y-1">
                                            <p className="text-[10px] font-bold text-teal-400 uppercase tracking-widest">Active Status</p>
                                            <p className="text-lg font-black tracking-tight">Verified Agent</p>
                                        </div>
                                        <ShieldCheck className="text-teal-400" size={32} />
                                    </div>
                                    <div className="flex items-center gap-4 text-[12px] font-bold text-gray-400">
                                        <div className="flex items-center gap-1.5"><Mail size={12} /> abc@gmail.com</div>
                                        <div className="flex items-center gap-1.5"><Phone size={12} /> 99999 99999</div>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        {activeTab === 'properties' ? (
                            <div className="space-y-12 animate-fade-in-up">
                                {activeSidebarItem === 'manage-projects' ? (
                                    <ManageProjectsView onAddProject={() => onNavigate('project-listing')} />
                                ) : (
                                    <>
                                        ...
                                        {activeSidebarItem === 'manage-properties' ? (
                                            <div className="space-y-8 contents-wrapper">
                                                ...
                                                <div className="pt-10 space-y-8">
                                                    <div className="flex items-center justify-between">
                                                        <h3 className="text-sm font-black text-[#ff3d3d] uppercase tracking-wider relative inline-block pb-1">
                                                            Properties
                                                            <div className="absolute bottom-0 left-0 w-2/3 h-0.5 bg-[#ff3d3d]"></div>
                                                        </h3>
                                                        <div className="flex items-center gap-4">
                                ...
                                <div className="bg-[#F8FAFC] rounded-[40px] p-12 flex flex-col items-center justify-center text-center space-y-6 border border-gray-100 shadow-inner min-h-[400px]">
                                    ...
                                    <div className="space-y-2">
                                        <p className="text-gray-500 font-bold text-[15px]">
                                            Please subscribe package to view responses.
                                        </p>
                                        <button
